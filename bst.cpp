...
void bst::inOrderSuccessor(node* root, int & count)
{
...
2 DELETION CASES
CASE 1: The leaf has no child nodes. In that case set the according entry in the parent to null.
CASE 2: The leaf has a right child (there can't be a left child node because that would be smaller). In that case, replace the current left node with the smallest node of the right subtree.
*/
node* bst::removeSmallest(node* root)
{
...
//also have to handle the case where the node you pass has no right child, (node itself is the largest node)
...
node* bst::removeSmallest(node* root)
{
  ...
  if (root->left == NULL && root->right == NULL) {
    return NULL; //same as delete root; root = NULL; return root;
  }
